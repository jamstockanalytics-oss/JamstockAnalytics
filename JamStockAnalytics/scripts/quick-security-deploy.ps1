# =============================================
# QUICK SECURITY DEPLOYMENT SCRIPT
# =============================================
# This script deploys security fixes using provided environment variables

param(
    [string]$SupabaseHost = if ($env:SUPABASE_HOST) { $env:SUPABASE_HOST } else { "your-supabase-host" },
    [string]$SupabasePassword = if ($env:SUPABASE_PASSWORD) { $env:SUPABASE_PASSWORD } else { "your-password" },
    [string]$Location = if ($env:LOCATION) { $env:LOCATION } else { "your-location" }
)
...
function Run-SqlScript {
    ...
    if (Test-Path "scripts/$ScriptName") {
        try {
            psql -h $SupabaseHost -U postgres -d postgres -f "scripts/$ScriptName"
