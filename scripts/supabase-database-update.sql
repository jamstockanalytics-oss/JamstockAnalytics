...
-- Social Media Sharing System & Chart Features
-- =====================================================
...
CREATE TABLE IF NOT EXISTS public.chart_data_cache (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  content_type VARCHAR(50) NOT NULL, -- 'market_summary', 'stock_data', 'analysis'
  content_id VARCHAR(255) NOT NULL,
  chart_type VARCHAR(20) NOT NULL, -- 'line', 'bar', 'pie', 'area'
...
-- Market Summary Extensions
ALTER TABLE public.market_summary ADD COLUMN IF NOT EXISTS chart_data JSONB DEFAULT '{}';
ALTER TABLE public.market_summary ADD COLUMN IF NOT EXISTS sentiment_score DECIMAL(3,2);
ALTER TABLE public.market_summary ADD COLUMN IF NOT EXISTS volatility_index DECIMAL(5,2);
ALTER TABLE public.market_summary ADD COLUMN IF NOT EXISTS market_trend VARCHAR(20) DEFAULT 'neutral';

-- Stock Data Extensions
ALTER TABLE public.stock_data ADD COLUMN IF NOT EXISTS chart_data JSONB DEFAULT '{}';
ALTER TABLE public.stock_data ADD COLUMN IF NOT EXISTS technical_indicators JSONB DEFAULT '{}';
ALTER TABLE public.stock_data ADD COLUMN IF NOT EXISTS social_sentiment DECIMAL(3,2);
...
CREATE TABLE IF NOT EXISTS public.user_activity_analytics (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID REFERENCES public.users(id) ON DELETE CASCADE,
  activity_type VARCHAR(50) NOT NULL, -- 'share', 'view', 'interact', 'export'
...
-- =====================================================
-- 10. SAMPLE DATA AND INITIALIZATION
-- =====================================================
...
COMMENT ON TABLE public.social_share_events IS 'Tracks all social media sharing events by users';
COMMENT ON TABLE public.user_social_preferences IS 'Stores user preferences for social media sharing';
COMMENT ON TABLE public.user_chart_preferences IS 'Stores user preferences for chart display and behavior';
COMMENT ON TABLE public.chart_data_cache IS 'Caches chart data for improved performance';
COMMENT ON TABLE public.user_profile_extensions IS 'Extended user profile data including preferences';
COMMENT ON TABLE public.user_activity_analytics IS 'Tracks user activity for analytics and insights';
COMMENT ON TABLE public.content_performance_analytics IS 'Tracks content performance metrics';

COMMENT ON FUNCTION get_user_social_stats(UUID) IS 'Returns comprehensive social sharing statistics for a user';
COMMENT ON FUNCTION update_content_performance(VARCHAR, VARCHAR, VARCHAR) IS 'Updates content performance metrics when user interacts with content';
COMMENT ON FUNCTION cleanup_expired_chart_cache() IS 'Removes expired chart data from cache';
...
DO $$
BEGIN
  RAISE NOTICE 'JamStockAnalytics Database Update Script Completed Successfully!';
  ...
  RAISE NOTICE 'Database is ready for the updated JamStockAnalytics application!';
END $$;
